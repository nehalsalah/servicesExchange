...
public class MainCategoriesCallback implements retrofit2.Callback<List<SkillDTO>> {
    @Override
    public void onResponse(Call<List<SkillDTO>> call, Response<List<SkillDTO>> response) {
        if (response.body() != null)
            EventBus.getDefault().post(new CustomEvent(CustomEventType.MAIN_CATEGORIES_LIST, response.body()));
    }
