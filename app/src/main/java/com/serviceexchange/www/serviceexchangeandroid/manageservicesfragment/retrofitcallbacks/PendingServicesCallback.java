...
public class PendingServicesCallback implements retrofit2.Callback<java.util.List<com.serviceexchange.www.serviceexchangeandroid.pojos.TransactionDto>> {
    @Override
    public void onResponse(Call<List<TransactionDto>> call, Response<List<TransactionDto>> response) {
        if (response.body() != null)
            EventBus.getDefault().post(new CustomEvent(CustomEventType.PENDING_SERVICES_TRANS, response.body()));
        else
    ...
    public void onFailure(Call<List<TransactionDto>> call, Throwable t) {
        EventBus.getDefault().post(new CustomEvent(CustomEventType.FAILED_REQUEST, t.toString()));
    }
}
